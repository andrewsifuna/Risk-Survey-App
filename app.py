...
sections = [
    ...
    "Employees","Machinery & Engineering Systems","Health & Safety","Fire Protection","Fire Services",
...
# =========================
    ...
    d["fire_water_availability"] = st.selectbox(
        ...
        ["Yes", "No"],
        index=["Yes", "No"].index(
            d.get("fire_water_availability", "Yes")
        )
    )
    ...
    d["fuel_storage"] = st.text_area(
        "Fuel / Gas Storage (diesel, LPG, etc.)",
        d.get("fuel_storage", "")
    )
    ...
    # SMART RISK ALERT
    # =========================
    if d.get("power_reliability") == "Frequent Outages" and d.get("backup_power") == "No":
    ...
    d["num_employees"] = st.number_input(
        "Number of Employees",
        min_value=0,
        value=int(d.get("num_employees", 0))
    )
    ...
    d["breakdown_history"] = st.text_area(
        "Breakdown History (last 3 years)",
        d.get("breakdown_history", "")
    )
    ...
    d["ppe_usage"] = st.selectbox(
        ...
        ["High", "Moderate", "Low"],
        index=["High", "Moderate", "Low"].index(
            d.get("ppe_usage", "High")
        )
    )
    ...
    d["signage"] = st.selectbox(
        ...
        index=["Adequate", "Limited", "None"].index(
            d.get("signage", "Adequate")
        )
    )
    ...
    d["fire_detection"] = st.selectbox(
        ...
        index=["None", "Smoke Detectors", "Heat Detectors", "Automatic Fire Alarm System"].index(
            d.get("fire_detection", "None")
        )
    )

    d["alarm_monitoring"] = st.selectbox(
        "Alarm Monitoring",
        ["None", "Local Alarm Only", "Connected to Security", "Connected to Fire Brigade"],
        index=["None", "Local Alarm Only", "Connected to Security", "Connected to Fire Brigade"].index(
            d.get("alarm_monitoring", "None")
        )
    )
    ...
    d["sprinkler_coverage"] = st.selectbox(
        "Sprinkler Coverage",
        ["Full", "Partial", "None"],
        index=["Full", "Partial", "None"].index(
            d.get("sprinkler_coverage", "None")
        )
    )
    ...
    d["electrical_fire_risk"] = st.selectbox(
        ...
        ["Low", "Medium", "High"],
        index=["Low", "Medium", "High"].index(
            d.get("electrical_fire_risk", "Medium")
        )
    )
    ...
    d["access_control"] = st.selectbox(
        ...
        index=["None", "Manual (Guards)", "Controlled Entry", "Biometric / Card Access"].index(
            d.get("access_control", "Manual (Guards)")
        )
    )
    ...
    d["key_control"] = st.selectbox(
        ...
        index=["Poor", "Moderate", "Strict"].index(
            d.get("key_control", "Moderate")
        )
    )
    ...
    d["lighting"] = st.selectbox(
        "External Lighting",
        ["Poor", "Adequate", "Good"],
        index=["Poor", "Adequate", "Good"].index(
            d.get("lighting", "Adequate")
        )
    )
    ...
    d["storage_housekeeping"] = st.selectbox(
        ...
        ["Good", "Average", "Poor"],
        index=["Good", "Average", "Poor"].index(
            d.get("storage_housekeeping", "Good")
        )
    )
    ...
    d["antivirus"] = st.selectbox(
        ...
        ["None", "Basic", "Advanced"],
        index=["None", "Basic", "Advanced"].index(
            d.get("antivirus", "Basic")
        )
    )
    ...
    d["cyber_incidents"] = st.selectbox(
        ...
        ["None", "Minor", "Major"],
        index=["None", "Minor", "Major"].index(
            d.get("cyber_incidents", "None")
        )
    )

    d["incident_details_it"] = st.text_area(
        "Details of Cyber / IT Incidents",
        d.get("incident_details_it", "")
    )
    ...
    d["disposal_method"] = st.selectbox(
        ...
        index=["Open Dumping", "Municipal Collection", "Recycling", "Licensed Contractor"].index(
            d.get("disposal_method", "Municipal Collection")
        )
    )
    ...
    d["process_description"] = st.text_area(
        "Describe Core Business Process / Operations",
        d.get("process_description", "")
    )
    ...
    d["annual_turnover"] = st.number_input(
        "Annual Turnover (KES)",
        value=float(d.get("annual_turnover", 0))
    )

    d["gross_profit"] = st.number_input(
        "Gross Profit (KES)",
        value=float(d.get("gross_profit", 0))
    )

    d["fixed_costs"] = st.number_input(
        "Fixed Costs (KES)",
        value=float(d.get("fixed_costs", 0))
    )
    ...
    d["indemnity_period"] = st.selectbox(
        ...
        [3, 6, 9, 12, 18, 24],
        index=[3, 6, 9, 12, 18, 24].index(
            int(d.get("indemnity_period", 12))
        )
    )
    ...
    d["estimated_downtime"] = st.number_input(
        ...
        value=float(d.get("estimated_downtime", 0))
    )
    ...
    d["key_suppliers"] = st.text_area(
        "Key Suppliers (dependency risk)",
        d.get("key_suppliers", "")
    )

    d["key_customers"] = st.text_area(
        "Key Customers",
        d.get("key_customers", "")
    )
    ...
    d["backup_plan"] = st.selectbox(
        ...
        index=["None", "Basic", "Comprehensive"].index(
            d.get("backup_plan", "Basic")
        )
    )
    ...
    d["recovery_time"] = st.number_input(
        ...
        value=float(d.get("recovery_time", 0))
    )
...
# =========================
    ...
    d["sum_insured"] = st.number_input(
        "Declared Sum Insured (KES)",
        value=float(d.get("sum_insured", total_value))
    )
    ...
    d["estimated_loss"] = st.number_input(
        "Estimated Likely Loss (KES)",
        value=float(d.get("estimated_loss", 0))
    )

    d["pml"] = st.number_input(
        "Probable Maximum Loss (PML)",
        value=float(d.get("pml", 0))
    )

    d["mpl"] = st.number_input(
        "Maximum Possible Loss (MPL)",
        value=float(d.get("mpl", 0))
    )
